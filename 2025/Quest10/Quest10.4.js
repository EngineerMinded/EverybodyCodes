...
class Grid {
     ...
     **************************************************************/
    part3Moves(i, j, sheepMove, gridcopy) {
        console.log(`Part 3 move at (${i}, ${j}) with sheepMove ${sheepMove}`);
        let cloneGrid = gridCopy
        ...
        cloneGrid.grid.moveSheepRow(sheepMove, i, j);
        if (!(cloneGrid.grid.sheepStillRemains())) {
            return 0;
        }
        ...
        for (let k = 0; k < cloneGrid.grid[0].length; k++) {
            if (k != sheepMove) {
                
                if (i >= 1 && j >= 2  &&  cloneGrid.lowestSheepLevel(i - 1) <= j ) {totalfinishes += clone.part3Moves(i - 1, j - 2, k);}
